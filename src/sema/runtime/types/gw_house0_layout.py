...
class GwHouse0Layout(SemaType):
    ...
    def check_axiom_1(self) -> Self:
        """
        Axiom 1: GlobalIdUniqueness ShNode.ShNodeId, Component.ComponentId,
        DataChannel.Id, DerivedChannel.Id and GNode.GNodeId SHALL each be globally
        unique; no id value SHALL appear in more than one of these sets.
        """
        ids: list[str] = []
    ...
    def check_axiom_2(self) -> Self:
        """
        Axiom 2: CoreShNodesExistenceAndActorClass ShNodes SHALL contain a node with
    ...
    def check_axiom_3(self) -> Self:
        """
        Axiom 3: CommandNodesExistenceAndActorClass ShNodes SHALL contain a node with
        each of the following Name / ActorClass pairs, and no additional ShNode with any
    ...
    def check_axiom_4(self) -> Self:
        """
        Axiom 4: CircuitHeatCallChannel For each circuit in Hydronic.ZoneCallCircuits,
    ...
    def check_axiom_5(self) -> Self:
        """
        Axiom 5: PrimaryFlowSourceChannelAgreement The "primary-flow" channel SHALL
        agree with Hydronic.PrimaryFlowSource. If PrimaryFlowSource is "Measured", a
        DataChannel named "primary-flow" SHALL exist and no DerivedChannel named
        "primary-flow" SHALL exist. If PrimaryFlowSource is "DerivedSiegSum", a
        DerivedChannel named "primary-flow" with Strategy "sum" SHALL exist and no
        DataChannel named "primary-flow" SHALL exist.
        """
        if self.hydronic is None:
            return self
    ...
    def check_axiom_6(self) -> Self:
        """
        Axiom 6: TransactivePowerChannel DerivedChannels SHALL contain exactly one channel
        ...
        for name in transactive[0].input_channel_names:
            if name in disabled:
                raise ValueError(
                    f"Axiom 6 (TransactivePowerChannel) failed: input '{name}' is in "
    ...
    def check_axiom_7(self) -> Self:
        """
        Axiom 7: RequiredSensing For each of the names "hp-odu-pwr", "hp-idu-pwr",
        "primary-pump-pwr", "store-pump-pwr", "dist-pump-pwr", "hp-lwt", "hp-ewt",
        "dist-swt", "dist-rwt", "store-hot-pipe", "store-cold-pipe", "buffer-hot-pipe",
        "dist-flow", and "store-flow": a channel with that Name SHALL exist in
        DataChannels or in DerivedChannels. (Kind-agnostic by design — a name may
        migrate from raw DataChannel to same-name DerivedChannel without touching this
        contract.)
        """
        if not self.sh_nodes:
            return self
    ...
    def check_axiom_8(self) -> Self:
        """
        Axiom 8: SiegManifoldChannels For each of the names "sieg-cold", "sieg-hot",
        "sieg-flow", and "sieg-send-flow": a channel with that Name SHALL exist in
        DataChannels or in DerivedChannels — the siegenthaler loop cannot be controlled
        or observed without its sensing channels.
        """
        if not self.sh_nodes:
            return self
        names = {c.name for c in (self.data_channels or [])} | {
    ...
    def check_axiom_9(self) -> Self:
        """
        Axiom 9: SystemModelEnergyChannels DerivedChannels SHALL include channels named
        ...
        for name, expected in expected_models.items():
            ...
            if type_name != expected:
                raise ValueError(
                    f"Axiom 9 (SystemModelEnergyChannels) failed: '{name}' must name "
    ...
    def check_axiom_10(self) -> Self:
        """
        Axiom 10: RequiredActuators a. ShNodes SHALL include nodes named "vdc-relay",
        "tstat-common-relay", "charge-discharge-relay", "hp-failsafe-relay",
        "hp-scada-ops-relay", "aquastat-ctrl-relay", "store-pump-relay",
        "hp-loop-on-off-relay", and "hp-loop-keep-send-relay", each with ActorClass
        "Relay", and nodes named "dist-010v" and "store-010v", each with ActorClass
        ...
        ActorClass "Relay". c. Each of "dist-010v" and "store-010v" SHALL have a
        ComponentId equal to the ComponentId of an i2c.dac.output.component.gt in
        Components.
        """
        actor_class_by_name = {
        ...
        component_id_by_name = {n.name: n.component_id for n in self.sh_nodes or []}
    ...
    def check_axiom_11(self) -> Self:
        """
        Axiom 11: RequiredHeatpumpEquipment ShNodes SHALL include nodes named "hp-odu"
        and "hp-idu" (the indoor unit exchanges refrigerant heat with the water), each
        with a ComponentId equal to the ComponentId of a Component in Components, and
        each with ActorClass "NoActor" unless it is the node Hydronic.HpCommandNodeName
        names, whose ActorClass CommandableHeatPump governs.
        """
    ...
    def check_axiom_12(self) -> Self:
        """
        Axiom 12: CommandableHeatPump a. If Hydronic.HpCommandNodeName is present, it
    ...
    def check_axiom_13(self) -> Self:
        """
        Axiom 13: PrefixClosedHandles Let the effective handle of an ShNode be its
    ...
    def check_axiom_14(self) -> Self:
        """
        Axiom 14: ActuatorLeaves Let the effective handle of an ShNode be its Handle if
    ...
    def check_axiom_15(self) -> Self:
        """
        Axiom 15: ComponentBinding Every Component in Components SHALL have its
    ...
    def check_axiom_16(self) -> Self:
        """
        Axiom 16: BoardResolution For every board-resident component in Components
        (i2c.relay.component.gt, i2c.dac.output.component.gt): its BoardComponentId
        SHALL equal the ComponentId of a scada.board.component.gt in Components; that
        board component's DeviceType SHALL match the DeviceType of a
        gw1.scada.device.type.gt record in DeviceTypes; and the component's board name
        (RelayName against the I2cRelays RelayNames for relays, DacName against the Dacs
        DacNames for DAC outputs) SHALL match a name in that record.
        """
        boards = {
            c.component_id: c
            for c in (self.components or [])
    ...
    def check_axiom_17(self) -> Self:
        """
        Axiom 17: BufferTank A House0 home has a buffer tank. For each depth i in 1..3 a
    ...
    def check_axiom_19(self) -> Self:
        """
        Axiom 19: CircuitWhitewireChannelResolution Every circuit's WhitewireChannelName
    ...
    def check_axiom_20(self) -> Self:
        """
        Axiom 20: DerivedChannelCreatorResolution a. Every channel's CreatedByNodeName
        ...
        for d in self.derived_channels or []:
            if d.created_by_node_name not in actor_class_by_name:
                raise ValueError(
                    "Axiom 20 (DerivedChannelCreatorResolution) failed (a): DerivedChannel "
            ...
            if actor_class_by_name[d.created_by_node_name] == "NoActor":
                raise ValueError(
                    "Axiom 20 (DerivedChannelCreatorResolution) failed (b): DerivedChannel "
    ...
    def check_axiom_21(self) -> Self:
        """
        Axiom 21: DataChannelNodeResolution a. Every channel's AboutNodeName in
    ...
    def check_axiom_22(self) -> Self:
        """
        Axiom 22: DerivedChannelInputsAcyclic a. Every name in a channel's
        ...
        for name, inputs in inputs_by_name.items():
            for input_name in inputs:
                if input_name not in data_names and input_name not in inputs_by_name:
                    raise ValueError(
                        "Axiom 22 (DerivedChannelInputsAcyclic) failed (a): DerivedChannel "
        ...
        for start in inputs_by_name:
            ...
            while frontier:
                ...
                if current == start:
                    raise ValueError(
                        "Axiom 22 (DerivedChannelInputsAcyclic) failed (b): DerivedChannel "
    ...
    def check_axiom_23(self) -> Self:
        """
        Axiom 23: ChannelNameUniqueness The Names of the channels in DataChannels and
        ...
        for name in [ch.name for ch in (self.data_channels or [])] + [
            ...
            if name in seen:
                raise ValueError(
                    "Axiom 23 (ChannelNameUniqueness) failed: more than one channel in "
    ...
    def check_axiom_24(self) -> Self:
        """
        Axiom 24: StoreTankTemps For each tank index N in 1..Hydronic.TotalStoreTanks
        ...
        if missing:
            raise ValueError(
                f"Axiom 24 (StoreTankTemps) failed: missing store tank channels {missing}."
            )
        return self
    ...
    def check_axiom_25(self) -> Self:
        """
        Axiom 25: WebServerNode ShNodes SHALL contain exactly one node named
    ...
    def check_axiom_27(self) -> Self:
        """
        Axiom 27: DisabledNamesResolve Every name in DisabledNodeNames SHALL equal the
        ...
        for name in self.disabled_node_names:
            if name not in node_names:
                raise ValueError(
                    f"Axiom 27 (DisabledNamesResolve) failed: DisabledNodeNames names "
                    f"'{name}', which is no ShNode."
                )
        for name in self.disabled_channel_names:
            if name not in channel_names:
                raise ValueError(
                    f"Axiom 27 (DisabledNamesResolve) failed: DisabledChannelNames names "
    ...
    def check_axiom_28(self) -> Self:
        """
        Axiom 28: DisabledNodesAreSensors Every name in DisabledNodeNames SHALL be the
        ...
        for name in disabled_nodes:
            if name not in capturing:
                raise ValueError(
                    f"Axiom 28 (DisabledNodesAreSensors) failed: '{name}' captures no "
                    "DataChannel."
                )
            if name in actuators:
                raise ValueError(
                    f"Axiom 28 (DisabledNodesAreSensors) failed: '{name}' is an actuator; "
        ...
        for c in self.data_channels or []:
            if (
                ...
                raise ValueError(
                    f"Axiom 28 (DisabledNodesAreSensors) failed: '{c.name}' is captured by "
    ...
    def check_axiom_29(self) -> Self:
        """
        Axiom 29: EnabledDerivedChannelsHaveLiveInputs Every DerivedChannel whose Name
        ...
        for d in self.derived_channels or []:
            ...
            if dead:
                raise ValueError(
                    f"Axiom 29 (EnabledDerivedChannelsHaveLiveInputs) failed: '{d.name}' is "
    ...
    def check_axiom_30(self) -> Self:
        """
        Axiom 30: ActuatorChannels a. For every ShNode RequiredActuators names — the
    ...
    def check_axiom_31(self) -> Self:
        """
        Axiom 31: PrimaryPumpActuators If Hydronic.PrimaryPumpOwner is "Scada",
        ShNodes SHALL include "primary-pump-failsafe-relay" and
        "primary-pump-scada-ops-relay" with ActorClass "Relay" and "primary-010v" with
        ActorClass "ZeroTenOutputer" and a ComponentId resolving to an
        i2c.dac.output.component.gt in Components, and each of the three SHALL have a
        DataChannel of its own Name in the ActuatorChannels shape. If it is "HeatPump",
        no ShNode and no DataChannel SHALL carry any of those three Names.
        """
        wanted = {
            "primary-pump-failsafe-relay": ("Relay", "RelayState"),
            "primary-pump-scada-ops-relay": ("Relay", "RelayState"),
            "primary-010v": ("ZeroTenOutputer", "VoltsTimesTen"),
        }
        nodes = {n.name: n for n in (self.sh_nodes or [])}
        channels = {c.name: c for c in (self.data_channels or [])}
        if str(self.hydronic.primary_pump_owner) == "HeatPump":
            present = sorted(
                name for name in wanted if name in nodes or name in channels
            )
            if present:
                raise ValueError(
        ...
        for name, (actor_class, telemetry) in wanted.items():
            node = nodes.get(name)
            if node is None or str(node.actor_class) != actor_class:
                raise ValueError(
                    "Axiom 31 (PrimaryPumpActuators) failed: PrimaryPumpOwner is "
                    f"Scada but no ShNode named {name!r} with ActorClass {actor_class}."
                )
            if (
                actor_class == "ZeroTenOutputer"
                and node.component_id not in dac_output_ids
            ):
                raise ValueError(
                    "Axiom 31 (PrimaryPumpActuators) failed: primary-010v ComponentId "
                    f"{node.component_id} is not an i2c.dac.output.component.gt."
                )
            channel = channels.get(name)
            if (
                channel is None
                or channel.about_node_name != name
                or channel.captured_by_node_name != name
                or str(channel.telemetry_name) != telemetry
            ):
                raise ValueError(
                    "Axiom 31 (PrimaryPumpActuators) failed: actuator "
                    f"{name!r} has no DataChannel of its own Name about and captured "
                    f"by itself with TelemetryName {telemetry}."
                )
        return self
    ...
    def check_axiom_32(self) -> Self:
        """
        Axiom 32: PrimaryPumpRecordAgreement Let the joined records be the
        hp.device.type.gt records in DeviceTypes whose DeviceType equals the DeviceType
        of the Component that "hp-odu" or "hp-idu" binds. If
        Hydronic.PrimaryPumpOwner is "Scada", no joined record SHALL have
        ...
        device_types = set()
        for n in self.sh_nodes or []:
            if n.name in ("hp-odu", "hp-idu") and n.component_id in component_by_id:
                device_type = getattr(
        ...
        for r in self.device_types or []:
            if not isinstance(r, HpDeviceTypeGt) or r.device_type not in device_types:
                continue
    ...
    def check_axiom_34(self) -> Self:
        """
        Axiom 34: HeatCallChannelBelongsToCircuit Every channel in DerivedChannels whose
