...
class GwHouse0Layout(SemaType):
    ...
    def check_axiom_8(self) -> Self:
        ...
        "sieg-flow", "sieg-send-flow", "hp-loop-on-off-relay", and
        "hp-loop-keep-send-relay": a channel with that Name SHALL exist in DataChannels
        or in DerivedChannels — the siegenthaler loop cannot be controlled or observed
        without its sensing and valve-observation channels.
        """
        if not self.sh_nodes:
            return self
        names = {c.name for c in (self.data_channels or [])} | {
